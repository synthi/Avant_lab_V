-- Avant_lab_V lib/controls.lua | Version 1.6.2
-- RELEASE v1.6: 
-- 1. WAVETABLE TUNING: Holding Track Select + E2 = Fine Tune Length (0.005).
...
Pages[6] = {
   ...
   key = function(n, z, s)
      ...
      elseif n==3 and z==1 then s.morph_fast_mode = not s.morph_fast_mode end
   end
}
...
Pages[7] = {
   ...
   key = function(n, z, s)
      ...
      elseif n==3 then
         if s.k1_held and z==1 then s.mixer_view = not s.mixer_view
         elseif z==1 then local t = s.tracks[s.track_sel]; t.speed = (t.speed or 1) * -1; Loopers.refresh(s.track_sel, s) end
      end
   end
}
   ...
   key = function(n, z, s)
      if n==2 and z==1 then
         ...
         fileselect.enter("/home/we/dust/audio/", function(file)
            ...
            if file ~= "cancel" then Loopers.load_file(s.tape_library_sel, file, s) end
         end)
      elseif n==3 and z==1 then
...
function Controls.key(n, z, state)
   ...
   if state.k1_held and z==1 then
      if n==2 then state.current_page = state.current_page - 1; if state.current_page < 1 then state.current_page = 10 end
      elseif n==3 then state.current_page = state.current_page + 1; if state.current_page > 10 then state.current_page = 1 end end
      return
   end
