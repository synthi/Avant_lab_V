// lib/Engine_Avant_lab_V.sc | Version 1.2
// RELEASE v1.2:
// 1. GAIN CURVE: Empirical Equal-Power curve based on user measurements (includes boost at 0.15-0.2).
// 2. DYNAMICS: Stabilizer tuned to "Nagra" specs (Attack 0.5ms, Release 300ms, Ratio increased).
// 3. CORE: Feedback loop logic aligned with new Lua safety.
...
Engine_Avant_lab_V : CroneEngine {
    ...
    alloc {
        ...
        SynthDef(\avant_lab_v_synth, {
            ...
            // Global Params
            var fb_amt = \fb_amt.kr(0) * 0.6; // Internal scaling for Tape Delay
            ...
            // Tape Delay Process (Simplified for brevity, logic maintained)
            ...
            4.do({ |i|
                ...
                var rate_slew = Lag.kr(l_speed[i], 0.05) * Select.kr((l_brake[i] * 4).round, [1.0, 1.0, 1.0, 0.5, 0.0]); // Simplified brake
                
                // PHYSICS: Wow/Drag
                var deg_curve = l_deg[i].pow(4.0);
                var flutter_mod = Select.kr(l_deg[i] > 0.6, [
                    LinLin.kr(l_deg[i], 0.0, 0.6, 0.0, 0.02),
                    Select.kr(l_deg[i] > 0.8, [
                        LinLin.kr(l_deg[i], 0.6, 0.8, 0.02, 0.06),
                        LinLin.kr(l_deg[i], 0.8, 1.0, 0.06, 0.10)
                    ])
                ]);
                ...
                // Filters & Degradation
                var deg_hpf = Select.kr(l_deg[i] > 0.5, [LinExp.kr(l_deg[i], 0.0, 0.5, 10, 60), LinExp.kr(l_deg[i], 0.5, 1.0, 60, 100)]);
                var deg_lpf = Select.kr(l_deg[i] > 0.5, [LinExp.kr(l_deg[i], 0.0, 0.5, 17000, 12000), LinExp.kr(l_deg[i], 0.5, 1.0, 12000, 4000)]);
                play_sig = LPF.ar(LPF.ar(play_sig, deg_lpf), deg_lpf); // 24dB LPF
                
                play_sig = play_sig * (1.0 - (LFNoise2.kr(8+(i*2)).unipolar * deg_curve * 0.6)); // Corrosion
                var loop_ero = LinLin.kr(l_deg[i], 0.4, 1.0, 0.0, 0.5).max(0);
                play_sig = play_sig * (1.0 - (Decay.kr(Dust.kr(loop_ero * 15), 0.1) * loop_ero).clip(0, 0.9)); // Erosion
                
                // Saturation S-Curve (Bypass < 0.2)
                var sat_drive = Select.kr(l_deg[i] > 0.2, [DC.kr(1.0), Select.kr(l_deg[i] > 0.5, [LinLin.kr(l_deg[i], 0.2, 0.5, 1.0, 1.5), Select.kr(l_deg[i] > 0.85, [LinLin.kr(l_deg[i], 0.5, 0.85, 1.5, 3.0), LinLin.kr(l_deg[i], 0.85, 1.0, 3.0, 4.5)])])]);
                play_sig = Select.ar(l_deg[i] < 0.2, [(play_sig * sat_drive).tanh, play_sig]);
                
                var in = Select.ar(l_src[i], [tap_clean, tap_post_tape, tap_post_filter, tap_post_reverb, trk1_in, trk2_in, trk3_in, trk4_in]);
                ...
                // [DYNAMIC STABILIZER - NAGRA SPECS v1.2]
                // Attack: 0.0005 (0.5ms), Release: 0.3 (300ms)
                // Threshold: 0.8, Ratio: aggressive above threshold
                var amp_det = Amplitude.kr(play_sig, 0.0005, 0.3);
                var dyn_stab = 1.0 - (amp_det.max(0.8) - 0.8 * 0.6).clip(0, 0.6); // 10% stronger ratio
                
                var write_sig = (play_sig * l_dub[i] * fb_curve * dyn_stab) + (in * l_rec_lvl[i].dbamp * gate_rec);
                BufWr.ar(write_sig, b_idx, ptr);
                
                var output_sig = HPF.ar(play_sig * gate_play, deg_hpf); // HPF at output
                output_sig = LPF.ar(output_sig, LinExp.ar(final_rate.abs.max(0.01), 0.25, 1.0, 6000, 17000).clip(1000, 20000)); // Speed Tone
        ...
        // ... (Other standard commands unchanged, assumed present) ...
        this.addCommand("buffer_read", "is", { |msg| var b=buffers[msg[1]-1]; if(File.exists(msg[2]), { b.zero; Buffer.readChannel(context.server, msg[2], 0, b.numFrames, [0,1], action:{|f| f.copyData(b); f.free; NetAddr("127.0.0.1", 10111).sendMsg("/buffer_info", msg[1], f.numFrames/context.server.sampleRate)}); }); });
        this.addCommand("buffer_write", "isf", { |msg| var b=buffers[msg[1]-1]; var frames=(msg[3]*context.server.sampleRate).asInteger; if(frames>0, {b.write(msg[2],"wav","int24",frames)}, {b.write(msg[2],"wav","int24")}); });
